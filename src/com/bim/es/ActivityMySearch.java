...
public class ActivityMySearch extends ActivityPub {
	public static final String MY_SEARCH_FILE_NAME = "entrez_seq_my_search.txt";
	...
	private void logDevice() {
		if (isFirstCreated) {
			DeviceES.save(this, Device.ACTION_MY_SEARCH, mRowAdapter
					.getSearchList().size()
					+ "");
		}
	...
	public List<ESearchSequence> loadSearch() {
		List<ESearchSequence> searchList = new ArrayList<ESearchSequence>();
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				ESearchSequence s = new ESearchSequence();
				if (s.parse(line)) {
					searchList.add(s);
				}
			}
	...
	private void delete() {
		...
		ArrayList<ESearchSequence> saveList = new ArrayList<ESearchSequence>();
		for (ESearchSequence s : mRowAdapter.getSearchList()) {
			if (s.isChecked()) {
				list.add(s);
			} else {
				saveList.add(s);
